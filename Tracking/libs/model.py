...
class AB3DMOT(object):			  	
	...
	def get_param(self, cat):
		...
		if self.det_name == 'CRN':	#Fine-tuned for CRN
			...
			if cat == 'car': 			algm, metric, thres, min_hits, max_age = 'hungar', 'iou_2d',   0.1, 1, 5
			elif cat == 'pedestrian': 	algm, metric, thres, min_hits, max_age = 'hungar', 'giou_2d', -0.5, 1, 5
			elif cat == 'truck': 		algm, metric, thres, min_hits, max_age = 'greedy', 'giou_3d', -0.6, 1, 5
			elif cat == 'trailer': 		algm, metric, thres, min_hits, max_age = 'hungar', 'giou_3d', -0.8, 3, 5
			elif cat == 'bus': 			algm, metric, thres, min_hits, max_age = 'hungar', 'giou_3d', -0.8, 1, 5
			elif cat == 'motorcycle':	algm, metric, thres, min_hits, max_age = 'hungar', 'giou_3d', -0.8, 3, 5
			elif cat == 'bicycle': 		algm, metric, thres, min_hits, max_age = 'hungar', 'giou_3d', -0.8, 3, 5
			else: assert False, 'cat name error: %s'%(cat)
